...
from downstream.synthesis.pennylane_op_jax import layer_circuit_to_pennylane_circuit
...
identit_params = {}
with open('identity_params.pkl', 'rb') as file:
    identit_params: dict = pickle.load(file)
...
def create_unitary_gate(connect_qubits):
    if len(connect_qubits) == 1:
        return [[{
            ...
            'params': np.zeros(3),  #  np.random.rand(3) * 2 * np.pi,
        }]]
...
class SynthesisModel():
    ...
    def construct_data(self, circuits, multi_process=False, n_random=10):
        ...
        def gen_data(circuit_info, n_qubits, index):
            ...
            found = False
            for layer_index, layer_gates in enumerate(layer2gates[1:]):
                # print(layer_index)
...
class SynthesisModelRandom():
    ...
    def choose(self, U, verbose = False):
        ...
        circuit_infos = gen_random_circuits(min_gate = 10 * n_qubits, max_gate=15 * n_qubits, backend = backend, gate_num_step = n_qubits * 2, n_circuits=1, two_qubit_gate_probs = [2, 4], optimize=True, reverse=False)
...
class SynthesisModelNN(SynthesisModel):
    ...
    def choose(self, U, verbose = False):
        ...
        for index in indices:
            ...
            candidate = [
                [create_unitary_gate(gate['qubits'])[0][0] for gate in layer_gates if len(gate['qubits']) == 2 ]
...
def _optimize(now_layers, new_layers, n_optimized_layers, U, lr, n_iter_no_change, n_qubits, allowed_dist, now_dist, remote = False):
    ...
    params, qml_dist, epoch_spent = find_parmas(n_qubits, optimized_layers, U @ unoptimized_U.T.conj(), max_epoch=500, allowed_dist=allowed_dist,
    ...
    qml_dists = [qml_dist]
    while len(epoch_finetunes) < 5:
        lr = max([qml_dist/5, 1e-2])  # 1e-1?
        params_finetune, qml_dist_finetune, epoch_finetune = find_parmas(n_qubits, total_layers, U, max_epoch=500, allowed_dist=allowed_dist,
                                                    n_iter_no_change=5, no_change_tolerance=qml_dist/100, random_params=False, lr=lr, verbose=False)
...
def synthesize(U, backend: Backend, allowed_dist=1e-5, heuristic_model: SynthesisModelNN = None, multi_process=True, 
               verbose=False, lagre_block_penalty=2, timeout = 5*3600, synthesis_log = None, n_unitary_candidates = 40):
    # heuristic_ratio = .5, 
    ...
    while now_dist > allowed_dist:
        ...
        if True: #iter_count != 0:
            ...
            for qubit_sets in itertools.product(*[backend.get_connected_qubit_sets(2) for depth in range(n_qubits)]):  # 
        ...
        if iter_count == 0:
            ...
            canditate_layers = [
                [copy.deepcopy(new_layer)] + candiate
                for candiate in canditate_layers
            ]
        ...
        for candidate_layer in canditate_layers:
            ...
            if multi_process:
                futures.append([candidate_layer, _optimize_remote.remote(
                    total_layers, candidate_layer, n_optimized_layers, U, lr, n_iter_no_change, n_qubits, allowed_dist, now_dist)])
            else:
                futures.append(
                    [candidate_layer ,_optimize(total_layers, candidate_layer, n_optimized_layers, U, lr, n_iter_no_change, n_qubits, allowed_dist, now_dist)])
        ...
        for future in futures:
            ...
            if verbose:
                print(candidate_layer['name'], candidate_layer['qubits'],
                      dist_decrement, now_dist, c_now_dist, subset_penalty, c_epoch, subp_time)
            
            if synthesis_log is not None:
                synthesis_log['print'].append((candidate_layer['name'], candidate_layer['qubits'],
                        dist_decrement, now_dist, c_now_dist, subset_penalty, c_epoch, subp_time))
        ...
        if synthesis_log is not None:
            synthesis_log['print'].append(('candiate selection cost', time.time() -
                    candiate_selection_start_time))
...
def synthesize_unitary_gate(gate: dict, backend: Backend, allowed_dist=1e-10, lagre_block_penalty=2, multi_process=True):
    ...
    if len(gate_qubits) <= 2:
        ...
        if len(gate_qubits) <= 4:
            ...
            # print('synthesis', len(gate_qubits), 'costs' ,time.time() - start_time, 's')
            # except:
            #     traceback.print_exc()
