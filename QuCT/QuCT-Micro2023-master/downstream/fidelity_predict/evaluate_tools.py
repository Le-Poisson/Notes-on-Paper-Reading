...
def get_excel(upstream_model, error_params_path_weight):
...
def plot_duration_fidelity(fig, axes, dataset, step=100, max_duration=0):
    ...
    axes.plot(duration_X, real_y, markersize=12,
              linewidth=2, label='real', marker='^')
    axes.plot(duration_X, predict_y, markersize=12,
              linewidth=2, label='predict', marker='^')
...
def plot_top_ratio(upstream_model, erroneous_pattern_weight):
    ...
    for top in range(1, 100, 1):
        ...
        for device, pattern_weights in erroneous_pattern_weight.items():
            path_table_size = len(
                upstream_model.device2path_table[device].keys())
            for pattern_weight in pattern_weights:
                if pattern_weight[1] < top * path_table_size:
                    total_find += 1

        find_ratio = total_find / \
            (len(upstream_model.erroneous_pattern.keys())
